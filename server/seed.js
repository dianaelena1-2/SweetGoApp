...
if (!cofetarieExistenta) {
    ...
    const infoProdus1 = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)
    `).run(cofetarieId, 'Tort Ciocolată', 'Tort cu cremă de ciocolată și fructe de pădure', 120, 'Torturi', 2, 1, 'frigorific', '2026-04-18',imagine1)
    adaugaIngredienteProdus(infoProdus1.lastInsertRowid, ['făină', 'zahăr', 'cacao', 'ouă', 'fructe de pădure', 'mascarpone', 'frișcă naturală'])
    console.log('✓ Produs: Tort Ciocolată (Torturi) - 120 lei')
    ...
    const infoProdus2 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Tort cu Fructe de Pădure', 'Blat pufos cu cremă de iaurt și fructe proaspete', 95, 'Torturi', 3, 1, 'frigorific', '2026-04-18', imagine2)
    adaugaIngredienteProdus(infoProdus2.lastInsertRowid, ['făină', 'zahăr', 'ouă', 'iaurt', 'fructe de pădure', 'gelatină'])
    console.log('✓ Produs: Tort cu Fructe de Pădure (Torturi) - 95 lei')
    ...
    const infoProdus3 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Prăjitură cu Ciocolată', 'Prăjitură pufoasă cu cremă de ciocolată neagră', 18, 'Prăjituri', 10, 1, 'masina', '2026-04-20', imagine3)
    adaugaIngredienteProdus(infoProdus3.lastInsertRowid, ['făină', 'zahăr', 'cacao', 'ouă', 'unt', 'ciocolată neagră'])
    console.log('✓ Produs: Prăjitură cu Ciocolată (Prăjituri) - 18 lei')
    ...
    const infoProdus4 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Prăjitură cu Lămâie', 'Prăjitură răcoritoare cu aromă de lămâie și glazură', 15, 'Prăjituri', 8, 1, 'bicicleta', '2026-04-20', imagine4)
    adaugaIngredienteProdus(infoProdus4.lastInsertRowid, ['făină', 'zahăr', 'ouă', 'lămâie', 'unt', 'lapte'])
    console.log('✓ Produs: Prăjitură cu Lămâie (Prăjituri) - 15 lei')
    ...
    const infoProdus5 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Macarons Vanilie', 'Set de 6 macarons cu cremă de vanilie', 42, 'Macarons', 5, 1, 'bicicleta', '2026-04-24', imagine5)
    adaugaIngredienteProdus(infoProdus5.lastInsertRowid, ['făină de migdale', 'zahăr', 'albș', 'vanilie', 'colorant'])
    console.log('✓ Produs: Macarons Vanilie (Macarons) - 42 lei')
    ...
    const infoProdus6 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Macarons Fructe de Pădure', 'Set de 6 macarons cu cremă de fructe de pădure', 45, 'Macarons', 4, 1, 'bicicleta', '2026-04-24', imagine6)
    adaugaIngredienteProdus(infoProdus6.lastInsertRowid, ['făină de migdale', 'zahăr', 'albuș', 'fructe de pădure', 'colorant'])
    console.log('✓ Produs: Macarons Fructe de Pădure (Macarons) - 45 lei')
    ...
    const infoProdus7 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Cupcake cu Ciocolată', 'Cupcake cu blat de ciocolată și topping cremos', 12, 'Cupcakes', 15, 1, 'masina', '2026-04-21', imagine7)
    adaugaIngredienteProdus(infoProdus7.lastInsertRowid, ['făină', 'cacao', 'zahăr', 'ouă', 'unt', 'ciocolată', 'lapte'])
    console.log('✓ Produs: Cupcake cu Ciocolată (Cupcakes) - 12 lei')
    ...
    const infoProdus8 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Cupcake cu Vanilie', 'Cupcake cu blat de vanilie și cremă de brânză', 11, 'Cupcakes', 12, 1, 'masina', '2026-04-21', imagine8)
    adaugaIngredienteProdus(infoProdus8.lastInsertRowid, ['făină', 'zahăr', 'ouă', 'vanilie', 'brânză cremă', 'lapte'])
    console.log('✓ Produs: Cupcake cu Vanilie (Cupcakes) - 11 lei')
    ...
    const infoProdus9 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Croissant simplu', 'Croissant clasic, pufos și auriu', 8, 'Croissante', 20, 1, 'bicicleta', '2026-04-27', imagine9)
    adaugaIngredienteProdus(infoProdus9.lastInsertRowid, ['făină', 'unt', 'zahăr', 'drojdie', 'lapte', 'ouă'])
    console.log('✓ Produs: Croissant simplu (Croissante) - 8 lei')
    ...
    const infoProdus10 = db.prepare(`
        INSERT INTO produse (cofetarie_id, numeProdus, descriere, pret, categorie, stoc, disponibil, transport_recomandat, data_expirare, imagine)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cofetarieId, 'Croissant cu ciocolată', 'Croissant umplut cu ciocolată neagră', 10, 'Croissante', 18, 1, 'bicicleta', '2026-04-27', imagine10)
    adaugaIngredienteProdus(infoProdus10.lastInsertRowid, ['făină', 'unt', 'zahăr', 'drojdie', 'ciocolată', 'lapte', 'ouă'])
    console.log('✓ Produs: Croissant cu ciocolată (Croissante) - 10 lei')
